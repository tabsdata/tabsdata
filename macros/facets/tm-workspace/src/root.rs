...
/// Public function for other modules to get workspace root
pub fn get_workspace_root_for_repository() -> PathBuf {
    let id = random_string(8);
    let log_crate_tm_workspace = LOG_CRATE_TM_WORKSPACE.unwrap_or("false") == "true";
    get_workspace_root(id, log_crate_tm_workspace).expect("Unable to determine workspace root")
}
