...
# In this example, we are obtaining the data from the file data.csv
# We then check that numeric columns are inferred its correct data type.
@td.publisher(
    td.LocalFileSource(os.path.join(ABSOLUTE_LOCATION, "data.csv"), format="csv"),
    "output",
)
def input_file_csv_dtype_inference(tf: td.TableFrame):
    schema = tf.schema
    assert (
