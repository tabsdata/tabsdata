...
# dropping the null values. The output is saved in
# output_file_with_none_parquet.parquet, and expected_result.json
...
@td.subscriber(
    name="output_file_with_none",
    tables="collection/table",
    destination=td.LocalFileDestination(
        os.path.join(DEFAULT_SAVE_LOCATION, "output_file_with_none_parquet.parquet")
    ),
)
def output_file_with_none(df: td.TableFrame):
    df.drop_nulls()
    return None
