...
from tabsdata._io.inputs.sql_inputs import OracleSource
...
def test_query_wrong_type_raises_type_error():
    ...
    with pytest.raises(SourceConfigurationError) as e:
        OracleSource(uri, query)
    assert e.value.error_code == ErrorCode.SOCE38
...
def test_wrong_credentials_type_raises_error():
    ...
    assert e.value.error_code == ErrorCode.SOCE39
...
def test_wrong_type_initial_values_raises_type_error():
    ...
    assert e.value.error_code == ErrorCode.SOCE37
