...
from tabsdata._io.inputs.sql_inputs import PostgresSource
...
def test_query_wrong_type_raises_type_error():
    ...
    with pytest.raises(SourceConfigurationError) as e:
        PostgresSource(uri, query)
    assert e.value.error_code == ErrorCode.SOCE32
...
def test_wrong_credentials_type_raises_error():
    ...
    assert e.value.error_code == ErrorCode.SOCE33
...
def test_wrong_type_initial_values_raises_type_error():
    ...
    assert e.value.error_code == ErrorCode.SOCE31
