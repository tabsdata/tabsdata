...
from tabsdata._io.output import MariaDBDestination, Output, build_output
...
def test_all_correct_destination_table_list():
    ...
    expected_dict = {
        MariaDBDestination.IDENTIFIER: {
            ...
            MariaDBDestination.CREDENTIALS_KEY: credentials._to_dict(),
            MariaDBDestination.IF_TABLE_EXISTS_KEY: "append",
        }
    }
    assert output._to_dict() == expected_dict
    assert output.__repr__()
    assert isinstance(build_output(output._to_dict()), MariaDBDestination)
