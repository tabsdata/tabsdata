...
from tests.testing_resources.test_output_s3_catalog.example import output_s3_catalog
...
def test_output_s3_catalog(tmp_path, s3_client):
    ...
    output_s3_catalog.output.uri = output_file
    context_archive = create_bundle_archive(
        output_s3_catalog,
        local_packages=ROOT_PROJECT_DIR,
        save_location=tmp_path,
    ...
    # Set up the catalog
    from tests.testing_resources.test_output_s3_catalog.example import (
        warehouse_path,
    )
    ...
    # Create the folder
    os.makedirs(warehouse_path)
    catalog = load_catalog(**output_s3_catalog.output.catalog.definition)
    catalog.create_namespace("testing_namespace")
