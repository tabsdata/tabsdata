...
from tests.testing_resources.test_input_oracle.example import input_oracle
from tests.testing_resources.test_input_oracle_initial_values.example import (
    input_oracle_initial_values,
)
from tests.testing_resources.test_output_oracle_driver_provided.example import (
    output_oracle_driver_provided,
)
from tests.testing_resources.test_output_oracle_list.example import output_oracle_list
...
@pytest.mark.requires_internet
@pytest.mark.slow
def test_input_oracle_initial_values(testing_oracle, tmp_path):
    logs_folder = os.path.join(LOCAL_DEV_FOLDER, inspect.currentframe().f_code.co_name)
    context_archive = create_bundle_archive(
        input_oracle_initial_values,
    ...
    expected_output_file = os.path.join(
        TESTING_RESOURCES_FOLDER,
        "test_input_oracle_initial_values",
        "expected_result1.json",
    )
    expected_output = read_json_and_clean(expected_output_file)
    assert output.equals(expected_output)
...
@pytest.mark.requires_internet
@pytest.mark.slow
def test_input_oracle_initial_values_stored_number_0(testing_oracle, tmp_path):
    logs_folder = os.path.join(LOCAL_DEV_FOLDER, inspect.currentframe().f_code.co_name)
    context_archive = create_bundle_archive(
        input_oracle_initial_values,
        local_packages=ROOT_PROJECT_DIR,
...
@pytest.mark.slow
@pytest.mark.oracle
def test_input_oracle_initial_values_stored_number_2(testing_oracle, tmp_path):
    logs_folder = os.path.join(LOCAL_DEV_FOLDER, inspect.currentframe().f_code.co_name)
    context_archive = create_bundle_archive(
        input_oracle_initial_values,
        local_packages=ROOT_PROJECT_DIR,
...
def test_output_oracle_list(tmp_path, testing_oracle):
    logs_folder = os.path.join(LOCAL_DEV_FOLDER, inspect.currentframe().f_code.co_name)
    context_archive = create_bundle_archive(
        output_oracle_list, local_packages=ROOT_PROJECT_DIR, save_location=tmp_path
    )
