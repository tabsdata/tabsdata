...
from tests.testing_resources.test_input_salesforce.example import input_salesforce
from tests.testing_resources.test_input_salesforce_initial_values.example import (
...
def test_input_salesforce(tmp_path):
    ...
    context_archive = create_bundle_archive(
        ...
        local_packages=[
            ROOT_PROJECT_DIR,
            "/Users/aleix/src/tabsdata/tabsdata-os/client/plugins",
        ],
...
def test_trigger_input(tmp_path):
    ...
    source.trigger_input(tmp_path)
    assert os.path.isfile(os.path.join(tmp_path, "0.parquet"))
    output = pl.read_parquet(os.path.join(tmp_path, "0.parquet"))
    output = clean_polars_df(output)
    assert output.is_empty()
