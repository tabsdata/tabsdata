...
@click.pass_context
def read_run(ctx: click.Context, name: str, collection: str, execution_id: str):
    ...
    execution_id = execution_id or logical_prompt(ctx, "ID of the run to read")
    click.echo(
        f"Reading information of run '{execution_id}' for function '{name}' in "
        f"collection '{collection}'"
    )
    click.echo("-" * 10)
    try:
        server: TabsdataServer = ctx.obj["tabsdataserver"]
        response = server.function_read_run(collection, name, execution_id)
...
def trigger(ctx: click.Context, name: str, collection: str, execution_plan_name: str):
    ...
    click.echo("-" * 10)
    try:
        execution_plan: ExecutionPlan = ctx.obj["tabsdataserver"].function_trigger(
            collection, name, execution_plan_name=execution_plan_name
        )
        click.echo("Function triggered successfully")
        click.echo(f"Execution id: {execution_plan.id}")
        dot = execution_plan.dot
        execution_plan_real_name = execution_plan.name
        if dot:
