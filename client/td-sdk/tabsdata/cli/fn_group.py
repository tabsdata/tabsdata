...
def read_run(ctx: click.Context, name: str, collection: str, execution_id: str):
    ...
    try:
        server: TabsdataServer = ctx.obj["tabsdataserver"]
        response = server.read_function_run(collection, name, execution_id)
...
@click.pass_context
def trigger(ctx: click.Context, name: str, collection: str, execution_name: str):
    """Trigger a function"""
    verify_login_or_prompt(ctx)
    name = name or logical_prompt(ctx, "Name of the function to be triggered")
    collection = (
    ...
    try:
        server: TabsdataServer = ctx.obj["tabsdataserver"]
        execution = server.trigger_function(
            collection, name, execution_name=execution_name
        )
