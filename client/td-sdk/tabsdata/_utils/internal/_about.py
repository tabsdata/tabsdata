...
def tdabout(package_name="tabsdata"):
    ...
    # Git Information
    for repo_name, repo_desc, repo_prefix in TABSDATA_REPOSITORIES:
        exists = metadata.get(f"X-Git-{repo_prefix}-Exists", "false")
        env[f"VERGEN_GIT_{repo_prefix}_EXISTS"] = exists

        if exists == "true":
            env[f"VERGEN_GIT_{repo_prefix}_NAME"] = metadata.get(
                f"X-Git-{repo_prefix}-Name", "?"
            )
            env[f"VERGEN_GIT_{repo_prefix}_DESCRIPTION"] = metadata.get(
                f"X-Git-{repo_prefix}-Description", "?"
            )
            env[f"VERGEN_GIT_{repo_prefix}_BRANCH"] = metadata.get(
                f"X-Git-{repo_prefix}-Branch", "?"
            )
            env[f"VERGEN_GIT_{repo_prefix}_TAG"] = metadata.get(
                f"X-Git-{repo_prefix}-Tag", "?"
            )
            env[f"VERGEN_GIT_{repo_prefix}_SHA"] = metadata.get(
                f"X-Git-{repo_prefix}-Commit-Short-Hash", "?"
            )
            env[f"VERGEN_GIT_{repo_prefix}_LONG_HASH"] = metadata.get(
                f"X-Git-{repo_prefix}-Commit-Long-Hash", "?"
            )
            env[f"VERGEN_GIT_{repo_prefix}_COMMIT_DATE"] = metadata.get(
                f"X-Git-{repo_prefix}-Commit-Date", "?"
            )
            env[f"VERGEN_GIT_{repo_prefix}_COMMIT_TIMESTAMP"] = metadata.get(
                f"X-Git-{repo_prefix}-Commit-Timestamp", "?"
            )
            author = metadata.get(f"X-Git-{repo_prefix}-Commit-Author", "?")
            if "<" in author and ">" in author:
                name, email = author.rsplit("<", 1)
                env[f"VERGEN_GIT_{repo_prefix}_COMMIT_AUTHOR_NAME"] = name.strip()
                env[f"VERGEN_GIT_{repo_prefix}_COMMIT_AUTHOR_EMAIL"] = email.rstrip(">")
            else:
                env[f"VERGEN_GIT_{repo_prefix}_COMMIT_AUTHOR_NAME"] = author
                env[f"VERGEN_GIT_{repo_prefix}_COMMIT_AUTHOR_EMAIL"] = "?"
            env[f"VERGEN_GIT_{repo_prefix}_COMMIT_MESSAGE"] = metadata.get(
                f"X-Git-{repo_prefix}-Commit-Message", "?"
            )
            env[f"VERGEN_GIT_{repo_prefix}_COMMIT_COUNT"] = metadata.get(
                f"X-Git-{repo_prefix}-Commit-Count", "?"
            )
            env[f"VERGEN_GIT_{repo_prefix}_DESCRIBE"] = metadata.get(
                f"X-Git-{repo_prefix}-Describe", "?"
            )
            env[f"VERGEN_GIT_{repo_prefix}_DIRTY"] = metadata.get(
                f"X-Git-{repo_prefix}-Dirty", "?"
            )
