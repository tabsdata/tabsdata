...
use td_objects::rest_urls::{FunctionVersionParam, FUNCTION_VERSION_GET};
use td_objects::types::function::FunctionVersionWithTables;
...
get_status!(FunctionVersionWithTables);

#[apiserver_path(method = get, path = FUNCTION_VERSION_GET, tag = FUNCTIONS_TAG)]
#[doc = "Show a function version"]
pub async fn read(
    State(state): State<Functions>,
    Extension(context): Extension<RequestContext>,
    Path(param): Path<FunctionVersionParam>,
