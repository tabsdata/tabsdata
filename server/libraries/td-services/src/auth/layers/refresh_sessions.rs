...
pub async fn refresh_sessions(
    SrvCtx(sessions): SrvCtx<Sessions<'_>>,
    Connection(conn): Connection,
) -> Result<(), TdError> {
    let mut conn_ = conn.lock().await;
    let mut conn = conn_.get_mut_connection()?;
    sessions.purge(Some(&mut conn)).await?; // erases expired sessions
    sessions.get(Some(&mut conn)).await?; // force the reload of the cache
    Ok(())
}
