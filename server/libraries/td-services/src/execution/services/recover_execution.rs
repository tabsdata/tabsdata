...
mod tests {
    ...
    #[td_test::test(sqlx)]
    async fn test_recover_execution(db: DbPool) -> Result<(), TdError> {
        ...
        // Create another execution that won't be recovered
        let unrecovered_execution = seed_execution(&db, &function_version_1).await;
        let unrecovered_transaction =
            seed_transaction(&db, &unrecovered_execution, &TransactionKey::try_from("S")?).await;

        let mut unrecovered_function_runs = vec![];
        for function_version in function_versions.iter() {
            let function_run = seed_function_run(
                &db,
                &collection,
            ...
            .await;
            unrecovered_function_runs.push(function_run);
        }
        ...
        let service = ExecutionRecoverService::new(
            db.clone(),
        ...
        assert!(execution.ended_on().is_none());
        assert_eq!(*execution.status(), ExecutionStatus::Running);

        // Assert transaction is recovered
        ...
        // Assert all function_runs are recovered
        ...
        assert_eq!(*function_run.status(), FunctionRunStatus::ReScheduled);

        // Assert all table_data_versions are recovered
        ...
        for function_version in function_versions[1..].iter() {
            ...
            assert_eq!(function_runs.len(), 1);
            let function_run = &function_runs[0];
            assert!(function_run.started_on().is_none());
            assert!(function_run.ended_on().is_none());
            assert_eq!(*function_run.status(), FunctionRunStatus::ReScheduled);
        ...
        // Assert unrecovered_execution is still scheduled
        let executions: Vec<ExecutionDBWithStatus> = queries
            .select_by::<ExecutionDBWithStatus>(&(unrecovered_execution.id()))?
            .build_query_as()
            .fetch_all(&db)
            .await
            .map_err(handle_sql_err)?;
        ...
        // Assert unrecovered_transaction is still scheduled
        let transactions: Vec<TransactionDBWithStatus> = queries
            .select_by::<TransactionDBWithStatus>(&(unrecovered_execution.id()))?
            .build_query_as()
            .fetch_all(&db)
            .await
            .map_err(handle_sql_err)?;
        ...
        let function_runs: Vec<FunctionRunDB> = queries
            .select_by::<FunctionRunDB>(&(unrecovered_execution.id()))?
            .build_query_as()
            .fetch_all(&db)
            .await
            .map_err(handle_sql_err)?;
