...
pub struct CopyRequest {
    pub(crate) source_target_pairs: Vec<(Location<Url>, Location<Url>)>,
...
pub struct AwsConfigs {
    pub(crate) access_key: Value,
    pub(crate) secret_key: Value,
    pub(crate) region: Option<Value>,
    pub(crate) extra_configs: Option<HashMap<String, String>>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AzureConfigs {
    pub(crate) account_name: Value,
    pub(crate) account_key: Value,
    pub(crate) extra_configs: Option<HashMap<String, String>>,
}
