...
use td_objects::rest_urls::TRANSACTION_CANCEL;
...
router! {
    state => { DatasetsState },
    paths => {{
        TRANSACTION_CANCEL => post(cancel_execution_plan),
    }}
}

#[api_server_path(method = post, path = TRANSACTION_CANCEL, tag = EXECUTION_TAG)]
#[doc = r#"
    Cancels an execution plan. This includes all functions that are part of the execution plan and
    all its dependants.
"#]
pub async fn cancel_execution_plan(
    State(dataset_state): State<DatasetsState>,
    Extension(context): Extension<RequestContext>,
    Path(uri_params): Path<TransactionUriParams>,
) -> Result<EmptyUpdateStatus, UpdateErrorStatus> {
    let request = context.update(uri_params, ());
    dataset_state
        .cancel_execution()
        .await
        .oneshot(request)
