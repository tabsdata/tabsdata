...
macro_rules! apiserver {
    ...
    $(.layer => $general_layer:expr $(,)?)*
        ...
        let $fn_name = async {
            ...
            $crate::logic::apiserver::ApiServerBuilder::new(addresses_if_any, router).build().await.unwrap()
        }.await;
    };
}
...
mod tests {
    ...
    use crate::logic::apiserver::macros::tests::test::Server;
    use crate::logic::apiserver::tests::wait_for_server;

    mod test {
        use crate::logic::apiserver::macros::tests::test;
        use crate::logic::apiserver::{localhost_address, ApiServer};
