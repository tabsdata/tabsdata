...
use crate::logic::apiserver::jwt::token::{AccessTokenClaims, EncodedToken, RefreshTokenClaims};
...
impl From<TokenError> for AuthorizeErrorStatus {
    fn from(error: TokenError) -> Self {
        let error: TdError = error.into();
        Self::from(error)
    }
}
