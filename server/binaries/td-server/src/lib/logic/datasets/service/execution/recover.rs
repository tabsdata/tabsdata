...
use crate::logic::datasets::layer::recover_request_to_state::{
    recover_request_to_state, reschedule_state,
};
use crate::logic::datasets::layer::select_execution_plan_failed_data_versions::select_transaction_failed_data_versions;
...
pub struct RecoverExecutionService {
    provider: ServiceProvider<UpdateRequest<TransactionId, ()>, (), TdError>,
}

impl RecoverExecutionService {
    /// Creates a new instance of [`RecoverExecutionService`].
    ...
    p! {
        provider(db: DbPool) -> TdError {
            service_provider!(layers!(
                ...
                from_fn(select_transaction_failed_data_versions),
...
mod tests {
    ...
    #[tokio::test]
    async fn test_tower_metadata_recover_service() {
        ...
        metadata.assert_service::<UpdateRequest<TransactionId, ()>, ()>(&[
            ...
            type_of_val(&select_transaction_failed_data_versions),
    ...
    async fn test_single() {
        ...
        let status: DsDataVersion = sqlx::query_as(SELECT_DATA_VERSION_STATUS)
            ...
            .await
            .unwrap();
        assert_eq!(status.status(), &DataVersionStatus::Scheduled);
    }
